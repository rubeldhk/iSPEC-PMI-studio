...
> discharged; resumption went through the Definition-of-Ready gate (EPIC-026), not by
> declaration.
>
> Delivered in **two stages**. Stage 1 (`epic/009-011-016-lifecycle-wave`, commit `cd81d70`)
> built everything EPIC-008 did not gate: the machine, baseline, versioning, diff, validation
> and approval. Stage 2 built the half that references `specifications` and
> `specification_versions` — the two models, the immutability trigger, and every endpoint —
> once EPIC-008's tables existed. The two lines were integrated before stage 2 began.
...
*Appended 2026-08-20 by `T194`, after both delivery stages landed. IDs continue from `T854` —
task IDs are unique repository-wide, and `T847`–`T854` were allocated to EPIC-001 and EPIC-005
while this Epic was being worked.*

### F1 · A baselined specification does not fork on edit — HIGH

*`BaselineService.editBaselined` implements FR-011a and `T099a` tests it. Nothing reaches it:
`PATCH /specifications/{id}` (EPIC-008 `T083a`) appends a version without consulting the lifecycle
state, so editing a **baselined** specification mutates the baseline's line rather than forking a
new draft. The requirement is built, tested, and unreachable — the shape `T462` and `T651` both
named. Found by reading US5 scenario 2 against the edit path rather than against the service.*

- [X] T855 [P] [US5] Unit tests asserting an edit to a **baselined** specification forks a new version born in `draft`, moves the specification to `draft`, and leaves the baselined version retrievable unchanged — and that an edit in any other state does NOT fork, in `backend/tests/unit/specifications/baseline-edit-fork.spec.ts` per FR-011a (partial)
- [X] T856 [US5] Consult the lifecycle state on the edit path so a baselined specification forks instead of being amended, in `backend/src/modules/specifications/specifications-read.service.ts` per FR-011a (partial; unit test: T855)
