...
- [ ] T1212 [US1] Decide whether two un-superseded baselines in one project should both read **current** — the screen now shows that they do. `T338i` makes supersession *"explicit, never inferred"*, so this is correct behaviour and possibly confusing presentation; inferring it in the UI would contradict the rule, so it needs a product decision rather than a fix
