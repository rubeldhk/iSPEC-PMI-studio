...
- [ ] T1213 [US1] Rebuild the frozen-version state on load — `RequirementRoom.tsx` keeps promoted versions in component state, so after a reload the approve form sends no members and the baseline is refused with *"a baseline must freeze at least one requirement version"*. The promotions survive in `promotedTo`; what is lost is the version and hash the page never re-reads. Found by the `T1212` walk
