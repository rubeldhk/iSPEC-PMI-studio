...
the concurrent-session gate that [plan.md](./plan.md) records as **FAIL**.
...
identifier per phase, suffixed within it"*. **76 tasks.**
...
- [ ] T436a Create the worktree `.claude/worktrees/epic-036-application-shell` from `main`, and label the session `EPIC-036 Application Shell`. Discharges the concurrent-session gate that [plan.md](./plan.md) records as **FAIL at planning** — `EPIC-033` `T337a` is the precedent
...
when a declared area is added, and that is only true if navigation, the route tree and the
reachability check are all derived from one declaration (`R-036-2`).

- [ ] T436f [P] Write failing tests for the area registry's invariants in `frontend/tests/unit/shell/areas.spec.ts` — every `id` unique, every `path` unique and leading `/`, no area in two groups, `declared: true` implies an `element`, `declared: false` implies none, and all four groups present in PMI-DOC-006 §4.1 order
- [ ] T436g Create the area registry in `frontend/src/shell/areas.ts` — the `Area` interface, the closed `AreaGroup` union and the `AREAS` list carrying **all eighteen** areas of PMI-DOC-006 §4.1 with **nine declared** (Home, Projects, Specifications, Plan & Tasks, Runs, QA & Releases, Architecture & Decisions, Governance, Workspace & Administration), per [contracts/shell-contract.md](./contracts/shell-contract.md) §1 (unit test: T436f)
- [ ] T436h [P] Write failing tests for navigation derivation in `frontend/tests/unit/shell/navigation-model.spec.ts` — declared areas only, grouped in registry order, and a group with no declared area omitted entirely rather than rendered as an empty heading (`FR-SHL-015`)
...
- [ ] T436j [P] Write failing tests for route-tree derivation in `frontend/tests/unit/shell/routes.spec.ts` — one route per declared area, no hand-written route, and `*` resolving to not-found
...
## Phase 3: User Story 1 — Every declared area is reachable from one navigation (P1) 🎯 MVP
...
**Independent test**: sign in and reach all nine declared areas from navigation without using
browser history or typing a URL; then open one area's address in a new tab and refresh it.

- [ ] T437a [P] [US1] Write failing tests for primary navigation in `frontend/tests/unit/shell/Navigation.spec.tsx` — four groups in §4.1 order, declared areas only, the current area marked, and navigation still present after selecting an area (`FR-SHL-010`, `FR-SHL-012`, `FR-SHL-013`)
...
- [ ] T437h [P] [US1] Write the failing `FR-SHL-016` reachability check in `frontend/tests/unit/shell/area-reachability.spec.tsx` — drive the **real** `App` through its real entry point and assert every `declared: true` area is reachable from primary navigation by clicking. Constitution XI Tier 1; `G-UX-01`'s navigation half
...
- [ ] T437j [US1] Mutation-verify `T437h` by removing one declared area's route and observing the check fail **naming that area**, then restore. `SC-SHL-001` states this as a number and `T200c` is the standard. The guard `DEF-010-001` did not have
- [ ] T437k [P] [US1] Write a failing test asserting **zero** undeclared areas appear in navigation — not disabled, not greyed, not a placeholder (`SC-SHL-002`, `UX-0060`) — in `frontend/tests/unit/shell/Navigation.spec.tsx`
- [ ] T437l [P] [US1] Write failing tests for addressability in `frontend/tests/unit/shell/addresses.spec.tsx` — every declared area resolves from its own address, a re-mount at that address lands in the same area, and back returns to the previous location (`SC-SHL-010`)
...
- [ ] T437n [P] [US1] Write a failing test asserting an address naming an **undeclared** area answers not-found rather than an empty area (`SC-SHL-011`, `FR-SHL-017`) in `frontend/tests/unit/shell/addresses.spec.tsx`
...
- [ ] T437p [P] [US1] Write a check asserting any declared area is reachable from any other in **at most two actions** (`SC-SHL-003`) in `frontend/tests/unit/shell/area-reachability.spec.tsx`
- [ ] T437q [P] [US1] Write the `SC-SHL-004` check in `frontend/tests/unit/shell/registry-extensibility.spec.ts` — adding a declared area to `AREAS` puts it in navigation and gives it a route **with no other file changed**. Assert it by construction over a synthetic registry, not by reading a diff
...
- [ ] T440a [P] [US4] Write failing tests for the drawer in `frontend/tests/unit/shell/NavigationDrawer.spec.tsx` — below the narrow breakpoint navigation collapses into a drawer opened by a persistent control, carrying the **full grouped list**, with every declared area reachable (`FR-SHL-054`)
...
- [ ] T440i [P] [US4] Write the `SC-SHL-007` check — every declared area reachable by keyboard alone at a 360px viewport, with all eighteen areas declared in a synthetic registry so the drawer is exercised at its worst case
...
- [ ] T441j Produce the Constitution XI Tier 2 **driven transcript** — sign in → Home → each of the nine declared areas → back → switch project — verbatim to `docs/accessibility/EPIC-036-shell-transcript.md`. `EPIC-029`'s reachability transcript is the precedent and the format
...
reaches every declared area from one navigation, and every area has an address that survives a
...
| Story | Passes when |
|---|---|
| US1 | all nine declared areas reached from navigation without history or a typed URL; one area's address opened fresh and refreshed; `T437j`'s mutation observed failing |
