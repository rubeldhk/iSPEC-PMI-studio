...
- [ ] T442o Correct the frontend suite figure in `specs/036-application-shell/closure.md`, per `Constitution IX` (partial) — it reports `frontend` **535 of 535 in 59 files**; the suite is **537 of 537 in 59 files** and has been since `T442h` added two assertions in Phase 11. `T442k` rewrote that document *specifically because its figures were stale*, and carried this one forward from Phase 10 without re-measuring. Re-measure every figure in the Suites paragraph rather than only this one, and state the date the measurement was taken so the next reader knows what it is a snapshot of
