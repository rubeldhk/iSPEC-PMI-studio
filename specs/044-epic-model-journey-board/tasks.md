...
- [X] T1614 Add the owner's assign/unassign control to each requirement row in `frontend/src/pages/Requirements.tsx` (a select of the project's active Epics plus *unassigned*, owner only via `getProject().ownerUserId`, calling `assignRequirementEpic`; a member without the grant sees the column and no control) with a failing expectation first in `frontend/tests/unit/pages/requirements.spec.tsx`, and pass `currentUserId` from `frontend/src/shell/area-views.tsx` per FR-EPB-023, US1/AC2 (partial)
- [X] T1615 Add a *Filter by stage* select beside *Filter by Epic* in `frontend/src/pages/SpecificationList.tsx` (options from `BoardRead.columns`, filtering on the derived stage of the row's Epic; *no Epic* rows match only the empty filter) with a failing expectation first in `frontend/tests/unit/pages/SpecificationList.spec.tsx` per FR-EPB-050 (partial)
- [X] T1616 State on the board screen in `frontend/src/pages/JourneyBoard.tsx`, beside the *derived from executions* line, that readiness conditions for this project will be configured under Governance → Constraints and that none are configured today, with a failing expectation first in `frontend/tests/unit/pages/journey-board.spec.tsx` per FR-EPB-046, US3/AC2 (partial)
- [X] T1617 Surface an execution whose command the stage configuration does not list: `evidenceFromExecutions` in `packages/epic-stage/src/evidence-executions.ts` returns the unrecognised command names, `EpicStageService` in `backend/src/modules/epics/epic-stage.service.ts` carries them on the card as `unrecognised: string[]`, and the card in `frontend/src/pages/JourneyBoard.tsx` reads *unrecognised command: …* rather than a wrong stage — failing tests first in `packages/epic-stage/tests/evidence-executions.spec.ts`, `backend/tests/unit/epics/epic-stage.service.spec.ts` and `frontend/tests/unit/pages/journey-board.spec.tsx` per spec §Edge Cases — the stage configuration changes (missing)
- [X] T1618 Record a slug collision on the child it renamed: `reconcileDecisions` in `backend/src/modules/epics/epic.service.ts` adds a finding *slug `<recorded>` collided with Epic N; created as `<recorded>-<number>`* and the Epic detail in `frontend/src/pages/EpicDetail.tsx` shows it in the Split section, with failing expectations first in `backend/tests/unit/epics/decision-reconcile.spec.ts` and `frontend/tests/unit/pages/epic-detail.spec.tsx` per spec §Edge Cases — the decomposition decision names a colliding slug (partial)
- [X] T1619 Record the deferral of FR-EPB-025's automatic specification-to-Epic binding as a dated note on the requirement in `specs/044-epic-model-journey-board/spec.md` (the execution-to-specification path is `EPIC-045`'s artifact sync; manual assignment and the row columns are delivered here) and name it in `specs/044-epic-model-journey-board/closure.md` §Work not done as a hand-off `EPIC-045` MUST pick up per FR-EPB-025 (partial)
