...
- [X] T1571 Implement `backend/src/modules/epics/epics.controller.ts` and `backend/src/modules/epics/epics.module.ts` (imports `EpicStoresModule`, `ExecutionsModule`, `AuditModule`, the grant and project services `OwnerGate` needs), and register `EpicsModule` in `backend/src/app.module.ts` (check: T1570)
