...
| `GET /v1/epics/{eid}` | member | — | `200` `Epic` with `requirements[]`, `specifications[]`, `children[]`, `parent?`, `decisions { createdBy, lastProcessed, decidedBy }`, `findings[]` (a slug collision recorded on the child it renamed — spec §Edge Cases, `T1618`) (`FR-EPB-063`, `FR-EPB-064`) |
