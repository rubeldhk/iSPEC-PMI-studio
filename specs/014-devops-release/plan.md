...
| The README documents both stacks | `T150p` | **Mutation** — remove the containerised section from `README.md`; the check must go red naming the missing stack (`T153g`). **It was invisible to `T153d` until `C-5`**: its assertions are `it('… (T150p)')` inside `describe('T452 · …')`, in a file the old hand-written `CHECK_FILES` did not name. Two blind spots, one check, and it had never been mutated because it was never in the inventory |
