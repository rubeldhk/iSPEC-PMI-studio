...
| 5 | extension at the version `pmi.project.context` reports; hooks registered; stock skills unchanged; the pinned tag satisfies the extension's `requires.speckit_version` (`FR-EXT-004`) | `.specify/extensions/pmi/extension.yml`, `.specify/extensions.yml`, the integration manifest digests, `engineTag` in `.pmi/project.json` | re-copy the extension from the bundle version `.pmi/project.json` names; merge the fragment; report any changed stock skill as `refused` (never repair it); report `refused` naming the tag and the range when the tag does not satisfy the range |
