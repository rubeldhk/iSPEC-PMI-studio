...
- [X] T1527 [P] [US4] Write failing unit tests `backend/tests/unit/connector/workstation-connection.service.spec.ts` (extend) — `pmi.health` from the setup skill records `extensionVersion`, `toolkitVersion` and the constitution state; a second call updates the same row
