...
## The question this plan cannot answer alone
...
**This plan proceeds under B**, because five of the eight success criteria are already stated as
counts and conformance ratios — they were written to be checked. Every task below therefore pairs
with a conformance check rather than a conventional unit test.

⚠️ **This is a Constitution reading, not a Constitution amendment.** If the owner prefers A, the
exemption should be recorded via `/speckit-constitution` rather than assumed here — and the same
question settles EPIC-003's `T088`/`T089` and EPIC-014's `T149`, which are open for the same reason.
...
**NEEDS CLARIFICATION**: none blocking. The Constitution V reading above is a decision, not an
unknown, and this plan states which way it proceeds.
...
| # | Gate | Status |
|---|------|--------|
| I | All code changes in this plan will be produced only via Spec Kit commands — no direct edits | PASS — and note Constitution I already **exempts** `.specify/**` and `specs/**`; artifacts this epic creates elsewhere need that exemption decided, which is `FR-RGP-009`'s governance index |
| II | Every requirement traces to a cited `SRS/` document; untraced items listed in Assumptions | PASS — with `FR-RGP-014`/`FR-RGP-015` declared as constitution-derived rather than SRS-derived, with a named back-fill owner |
...
| V | Every implementation task will carry a mandatory unit-test task, written to fail first | **PASS under reading B** — every task pairs with a conformance check. ⚠️ See "The question this plan cannot answer alone"; if the owner prefers reading A, this becomes PASS-with-recorded-exemption |
