# Epic Specification: Workflow & Tasks
...
> Specification, APPROVED; scope ruling T-106). This Epic implements **BR-0050**. The prior
...
Task generation from approved specifications, status tracking, and regeneration — completing the Requirement → Specification → Task lifecycle.
...
| Requirement |
|---|
| FR-020 generate tasks for an approved specification through the contract |

## User stories owned

- US4 — generate and manage tasks from a specification

## Success criteria owned

- SC-003 every task resolves back through its specification to a requirement

## Depends on

- EPIC-009 — the `approved` state; EPIC-003 — the engine
...
Task generation is gated on approval. Attempting it on a draft specification is refused stating that approval is required.
...
- [x] `specs/012-workflow-tasks/defects/` contains no open defect records
