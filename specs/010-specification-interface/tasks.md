...
- [X] T200e Route all five from the project view in `frontend/src/main.tsx` (unit test: `frontend/tests/unit/shell-page-routes.spec.tsx`) — `specifications` → `specification` → `tasks`, plus `storage` and `runs` → `review-session`, each with a way back. `T863`/`T864` is the precedent: the same defect one Epic earlier, one page at a time
