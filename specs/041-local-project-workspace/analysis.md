...
## Remediation (2026-09-03, same session)

Twelve of the thirteen findings were fixed on the requester's explicit approval. `/speckit-analyze`
is read-only by contract, so the edits were made after the record was written and are noted here
rather than by rewriting the findings above — the findings were true when found. **Resolved rows
carry `✅` in the ID cell**, the convention `DOR-09` reads: the finding and its severity stay in the
table, and it stops blocking. `K1` remains open by design.

| Finding | What changed |
|---|---|
| `C1` | `T1383` appended to Phase 2: the route-through generation proof — real `AppModule`, in-process worker with the real `JobPersistence`, submit through the route, dispose, re-boot, read back. Cites `SC-LPW-008`, `FR-LPW-040`, `FR-LPW-042` |
| `C2` | `T1355` extended: no `expiresAt` on the record type; a credential minted under a clock ninety days in the past still verifies. Cites `FR-LPW-028` |
| `C3` | `T1364` extended: `assurance` does not appear under `modules/{loop,policy,decisions,reviews}`. Cites `FR-LPW-031` |
| `C4` | `skillsPathFor(integration)` added to the bundle (`T1316`, `T1317`), documented in `contracts/project-files.md` §Skills path and `research.md` `R-041-9`; `T1339` and `T1344` cite `FR-LPW-006` and assert no integration is special-cased |
| `C5` | `T1352` asserts the written-file **set** equals the contract's enumeration. Cites `FR-LPW-011` |
| `U1` | `ConnectorScope` registry defined in `contracts/provisioning-api.md`; `T1359`/`T1360` build and test it; `connector-scope.ts` added to the plan's structure |
| `U2` | `PMI_PUBLIC_URL` added to configuration — `plan.md` Technical Context, `research.md` `R-041-2`, `contracts/project-files.md`, `T1310`, `T1311`, `T1337` |
| `I1` | `PMI_INITIALISE_WAIT_MS` named in `plan.md` Technical Context, `data-model.md` §1.1, `research.md` `R-041-2`, `T1310`, `T1311` |
| `I2` | `T1352` moved to `backend/tests/contract/project-files.spec.ts`; `plan.md` Testing and structure updated; the governance project keeps only the file-reading `T1310` |
| `I3` | `T1385`–`T1393` allocated for Phase Z as checkboxes with paths |
| `T1` | `SC-LPW-001` cited on `T1382`; `SC-LPW-002` on `T1343`, `T1351`; `SC-LPW-005` on `T1339`, `T1351` |
| `T2` | Terminology paragraph added to `contracts/provisioning-api.md`: *credential* in prose, *token* on the wire, neither renamed |
| `T3` | `T1384` added as the separate failing-first test for the four mounted components; `T1376` now names it |

Task count after remediation: **84** (`T1310`–`T1393`). `spec.md` was not touched: every fix
landed in the plan, the tasks or a contract, which is where each gap was.
