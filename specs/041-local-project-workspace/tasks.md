...
- [X] T1382 [P] Write `docs/operator-setup.md` §Local workspace and update `README.md` §Setup with the six variables, `uv` on the worker host, and the two-stack outcome table (conformance: `tests/governance/readme-conformance.spec.ts` `T452`, extended to require the six variables); run [quickstart.md](./quickstart.md) Scenarios 1–13 individually against **both** stacks and record each result with its stack in `specs/041-local-project-workspace/quickstart.md` §Results; measure and record the four performance targets of `research.md` §Performance, including create → open-in-agent under two minutes by a first-time user (`SC-LPW-001`)
...
- [X] T1388 **Two mutation observations recorded** in `specs/041-local-project-workspace/closure.md`: `T1352` fails when `backend/src/modules/projects/project-files.ts` is made to write the credential value (`SC-LPW-003`); `T1363` fails when the project-scope check in `backend/src/modules/connector/connector-auth.guard.ts` is removed (`SC-LPW-006`) — a contract test and an integration test observed failing under mutation, then restored (contract test: T1352; integration test: T1363)
- [X] T1389 **Constitution XII** — record in `specs/041-local-project-workspace/closure.md` that the commands producing this Epic's artifacts are **unregistered**, because the registry is mounted by `EPIC-043` behind this Epic's guard; this is the last Epic that can carry the row honestly
- [X] T1390 **ADRs**: confirm `adr/ADR-0030-local-first-execution-and-integration-contract.md` exists and `adr/ADR-0009-persistent-project-state-vs-ephemeral-execution.md`, `adr/ADR-0024-workspace-fabric-execution-modes.md` carry their dated amendments; move `adr/ADR-0017-interactive-workspace-vs-autonomous-sandbox.md` to **Accepted** and update the row in `adr/README.md`
- [X] T1391 Run `/speckit-converge`; append any remaining work to `specs/041-local-project-workspace/tasks.md`; triage `specs/041-local-project-workspace/defects/` leaving no open record; re-run `pnpm lint && pnpm -r typecheck && pnpm test && pnpm test:arch && pnpm test:governance`
- [X] T1392 Regenerate `governance/epic-stage-register.md` with `pnpm register:update` and confirm this Epic's derived stage matches its evidence
...
*Appended by `/speckit-converge` on 2026-09-04 (Constitution VII). Findings F1–F4 of that session; F5 (`SC-LPW-010` Tier 2 transcripts) is already the open `T1387` and is not duplicated; the `@pmi/mcp-server` package `.mcp.json` names is `EPIC-043`'s to publish.*

- [X] T1394 [P] Write failing component tests in `frontend/tests/unit/pages/project-run.spec.tsx` — a *Start run* control on `ProjectDetail` posts through `api.startRun(projectId, { mode, stopRange })`, shows the returned run's state and refreshes it until terminal, and surfaces the refusal body on `4xx`; the four `FR-SHL-060` states are distinguishable — then add the control and its progress region to `frontend/src/pages/Projects.tsx` (unit test: T1394) per FR-LPW-042, US4/AC3 (missing) — **HIGH**
- [X] T1395 [P] Write failing unit tests in `backend/tests/unit/projects/provisioning-audit.spec.ts` — the provisioning audit entry's `detail` names the path as the host sees it (never the write root), the agent integration and the script type beside outcome and failed step — then extend `append()` in `backend/src/modules/projects/provisioning.service.ts` (unit test: T1395) per FR-LPW-004 (partial) — **MEDIUM**
- [X] T1396 [P] Write failing unit tests in `backend/tests/unit/projects/provisioning-defaults.spec.ts` and extend `tests/governance/projects-root-config.spec.ts` — the default agent integration and script type are read from configuration (`PMI_DEFAULT_AGENT_INTEGRATION`, `PMI_DEFAULT_SCRIPT_TYPE` in `.env.example`, `docker-compose.yml` and `README.md` §Setup), falling back to `@pmi/workspace-bundle`'s `DEFAULT_AGENT_INTEGRATION` and `sh`; the backend still names no provider — then implement in `backend/src/modules/projects/projects-root.ts` and `provisioning.service.ts` (unit test: T1396; conformance: `tests/governance/projects-root-config.spec.ts`) per FR-LPW-005, contracts/provisioning-api.md §POST /projects (partial) — **MEDIUM**
- [X] T1397 [P] Write failing component tests in `frontend/tests/unit/pages/project-provisioning.spec.tsx` (extend) — the Local workspace panel renders its own loading status while the provisioning history loads and its own alert when that load fails, distinct from the project's — then extend `ProvisioningPanel` in `frontend/src/pages/Projects.tsx` (unit test: T1397) per FR-LPW-051 (partial) — **LOW**
