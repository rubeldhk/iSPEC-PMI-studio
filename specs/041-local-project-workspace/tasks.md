...
- [X] T1312 [P] Write failing unit tests for `ExecutionEnvironmentKind` in `packages/execution-contract/tests/environment-kind.spec.ts` — `descriptor.kind` is **required**, two members, and a `controlled-local` descriptor with `supportedLifecycles: ['ephemeral']` is a type error (`FR-LPW-030`, `R-041-4`)
