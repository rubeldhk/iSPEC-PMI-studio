...
## V5 — the delivered pages render from the system, against the running application

**Proves**: `SC-DS-005` · **Requirements**: `FR-DS-050`, `FR-DS-052` · **Constitution XI Tier 2**

This scenario is **the Epic's reachability evidence**, not a spot check. Constitution XI (v1.5.0)
requires an Epic delivering a user-facing journey to record a transcript **generated by the run**
against a running application before it may close.

```bash
pnpm --filter frontend dev      # then drive the journey against it
```

**Expected**: SignIn, Projects, Requirements and Traceability render from tokens alone; switching
between light and dark re-themes every one without an undefined token or text on a same-coloured
surface; at 200% text zoom nothing clips; at 360px width nothing overflows.

**The output is a transcript, not a verdict.** It must be emitted by the run and name what was
exercised — pages visited, theme switched, viewport and zoom applied — so that a reader can tell a
real pass from an assertion of one. **Hand-writing or editing this transcript is a constitution
violation**, the same rule `T709` was held to when `G-28-02` rejected its first attempt.
...
```bash
pnpm vitest run --project frontend tests/unit/design/token-sufficiency.spec.tsx
```

**Expected**: the fixture page — page header, form field, table, status pill, empty state — passes
the literal-value lint rule with zero violations, declares no new custom property, and every
`var(--…)` it references resolves to a token defined in `tokens.css`.

**Fails when**: the fixture reaches for a token that does not exist, naming the unresolved
reference. Mutation-verified with `var(--color-does-not-exist)`.

**Why this is a separate scenario from `V1`.** `V1` proves the tokens that *exist* are correct —
declared once, complete in both themes, contrasting properly. It cannot notice that the set is
**incomplete**, because a token nobody defined is a token `V1` never reads. `V6` is the only
scenario that fails when the scale has a hole in it, which is what `SC-DS-006` actually asks.

**Note**: this scenario exists because `SC-DS-006` had been cited by `V5`, `T900` and `T900b` —
none of which builds a new page (analysis `F3`). A criterion cited three times and tested zero
times looks better covered than one cited nowhere.

## What these scenarios do NOT prove

- That components are built on the **native element** `D-42` names for them. Every scenario here
  passes on a div-based reimplementation too. That obligation needs its own assertion — extending
  the state-coverage check, which already reads
  [contracts/components.md](./contracts/components.md), to assert each component renders the
  element its row names. Enforcing it by review would satisfy nothing: Constitution V is explicit
  that manual review does not discharge a check.
