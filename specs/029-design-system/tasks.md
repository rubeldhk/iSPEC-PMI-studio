...
**Task IDs**: `T865`–`T904` (corpus max was `T864`).
...
- [ ] T866 Create the directory structure `frontend/src/design/` with `tokens.css`, `themes.css` and `components/`, and import the stylesheets once at the application root in `frontend/src/main.tsx` — one import point, so no page can forget it
...
- [ ] T868 [P] Write the failing conformance check for the token layer in `tests/governance/design-tokens.spec.ts`: every token declared **exactly once**; category and naming shape per [contracts/tokens.md](./contracts/tokens.md); the `space` scale is a fixed ratio (`FR-DS-003`); the `type` scale has **at most seven** steps, each with size, line height and weight (`FR-DS-004`)
...
- [ ] T871 Define light and dark theme values in `frontend/src/design/themes.css`, with `prefers-color-scheme` as the default and a persistent explicit override (`FR-DS-011`; conformance check: T870)
...
- [ ] T879 [US2] Run `pnpm lint` across `frontend/src/**` and record every existing violation as the restyling backlog Phase 6 consumes — the four pages and two components were written with no tokens at all
...
- [ ] T880 [P] [US1] Write the accessibility harness in `frontend/tests/unit/a11y/axe.ts`, configured with **all five tags** — `wcag2a`, `wcag2aa`, `wcag21a`, `wcag21aa`, `wcag22aa` — and `axe.configure()` **explicitly enabling `target-size`**, without which the WCAG 2.2 delta is untested (research `R-029-2`)
...
- [ ] T883 [US1] Run the harness over each delivered page in `frontend/tests/unit/a11y/pages.spec.tsx` — SignIn, Projects, Requirements, Traceability — asserting zero violations (`FR-DS-031`, `SC-DS-001`; test harness: T880)
- [ ] T884 [P] [US1] Write the conformance check for the manual accessibility record in `tests/governance/design-tokens.spec.ts`: `docs/accessibility/EPIC-029-manual-pass.md` exists and names a **screen reader, its version, and at least one journey**. A file saying only "passed" MUST fail — that is the difference between a transcript and a tick (`FR-DS-034`, `SC-DS-008`)
...
- [ ] T890 [US3] Record decision **`D-42`** (build vs adopt) in `specs/_shared/decisions/D-42-component-library-build-vs-adopt.md`, with security review recorded if a dependency is adopted (`PP-008`, `PMI-DOC-005` `RULE-05`). **No component is implemented before this lands**
...
- [ ] T900 Verify the restyled pages at the **minimum viewport (360×640)** and at **200% text zoom** with no clipping or overflow, and record the result in [quickstart.md](./quickstart.md) `V5` (`FR-DS-040`, research `R-029-4`)
