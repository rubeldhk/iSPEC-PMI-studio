...
- [X] T462 Create `backend/src/modules/engines/engines.module.ts` and import it in `backend/src/app.module.ts` per FR-019 (partial) — `EngineRegistryService` and `EngineResolverService` are built and tested but nothing outside their own directory constructs them, so they are unreachable from the running API (unit test: `backend/tests/unit/engines/engines.module.spec.ts`)
- [X] T463 Implement a Prisma-backed `EngineRegistrationStore` in `backend/src/modules/engines/engine-registration.store.ts`, or record why the table stays unwritten, per T034 (partial) — the port has no implementation, so `recordRegistrations()` no-ops and `engine_registrations` is never written (unit test: `backend/tests/unit/engines/engine-registration.store.spec.ts`)
...
- [X] T465 Reconcile the fixture adapter path: T037 names `engine-adapters/fixture/src/fixture.adapter.ts`, the code lives at `src/index.ts` (partial) — correct the record or move the file (unit test: `engine-adapters/fixture/tests/unit/fixture.spec.ts`)

## Phase 8: Convergence *(appended 2026-08-19 by `/speckit-converge`)*

- [X] T688 Run the integration suite in CI, or record a decision with a named owner for why it is not run, per `SC-008` (partial) — `.github/workflows/ci.yml` runs `test:unit`, `test:arch`, `test:contract` and `test:governance`; `test:integration` is absent, so `backend/tests/integration/engine-swap.spec.ts` and `engine-default.spec.ts` — the only demonstrations of this epic's sole owned success criterion, and of `FR-018` against a real database — never execute on a push to the default branch. Both were run at closure and passed; nothing re-runs them. GitHub runners provide a container runtime, so Testcontainers is available (unit test: n/a — this is pipeline configuration, verified by the workflow running green with the suite included)
