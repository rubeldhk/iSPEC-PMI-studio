...
## Phase 8: Convergence — `DEF-001-004` *(appended 2026-08-20 by the first local UAT)*

**Why this phase exists**: the first UAT run watched a 401 and a 500 leave the API and be recorded
as `"status":200` at `info` level.
[`DEF-001-004`](./defects/DEF-001-004-failed-requests-are-logged-as-200.md) has the verbatim log
lines beside the `curl -i` responses. The interceptor's error arm is deliberate and its reasoning
is right — *"the requests worth measuring most are the ones that failed"* — but `finish()` reads
...
## A documented code for an unbound governance seam *(added 2026-08-29)*

*`DEF-033-002`'s third item, and this Epic's because it owns
[`platform-api.md`](../_shared/contracts/platform-api.md). Two Epics had each
declined to invent a status code for this, correctly — `DEF-008-001` is what happens when an Epic
that does not own the contract invents one. The gap was real: the status table had no 5xx entry at
all, so every unbound-seam refusal reached a user as `internal_error`.*

- [X] T1195 Add `governance_seam_unbound` (503) to `platform-api.md` and `backend/src/core/errors.ts`, and make `PolicyUnavailableError` and `EvidenceSourceUnavailableError` carry it *(unit test: `backend/tests/unit/core/governance-seam-unbound.spec.ts`; verified live — `POST /rooms/requirement/:id/decide` now answers `503` with "the PolicyProvider seam is unbound — EPIC-031 supplies it", where it previously answered `500` "An unexpected error occurred.")*
