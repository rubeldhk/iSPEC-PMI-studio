...
- [ ] T1244 [P] [US1] Write the failing integration test for the route in `backend/tests/integration/context-assembly-route.spec.ts` against PostgreSQL — the five outcomes in [contracts/context-api.md](./contracts/context-api.md), including `503` when the index is unavailable (`SC-CTX-008`)
...
- [ ] T1247 [P] [US2] Write failing unit tests for source references in `backend/tests/unit/context-provenance.spec.ts` — every item names its source and version, and **no item carries content** (`FR-CTX-040`, `FR-CTX-041`, `SC-CTX-002`)
