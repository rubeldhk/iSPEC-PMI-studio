...
- [ ] T1237 [P] [US1] Write failing unit tests for assembly inputs in `backend/tests/unit/context-assembly.spec.ts` — the objective, actor, role, permissions, classification, relevance and budget are all consulted and recorded (`FR-CTX-030`, `FR-CTX-031`, `FR-CTX-032`, `SC-CTX-001`)
...
- [ ] T1239 [P] [US1] Write failing unit tests for the unclassified default in `backend/tests/unit/context-classification.spec.ts` — a source type with no `SourceClass` is **excluded**, never admitted (`FR-CTX-034`, `SC-CTX-007`); `FR-GEL-062`'s rule that a default which permits is invisible
