...
- [ ] T1018 Create `packages/execution-contract/` mirroring `packages/engine-contract`'s layout — `package.json`, `tsconfig.json`, `src/index.ts`, `tests/` — and register it in the workspace and `vitest.workspace.ts` as project `execution-contract`
- [ ] T1019 [P] Add `backend/src/modules/executions/` and its test directories under `backend/tests/{unit,contract,integration}/executions/`
- [ ] T1020 [P] Create `specs/037-governed-execution-registry/defects/` with `.gitkeep` (Constitution VI — the sole defect intake for this Epic)
...
- [ ] T1021 Write failing contract tests for the semantic operation surface in `packages/execution-contract/tests/contract-shape.spec.ts` — seven operations, their refusal conditions, and the rule that **no operation mutates** (unit test: itself; conformance for T1022)
- [ ] T1022 Define the provider-neutral semantic contract in `packages/execution-contract/src/contract.ts` per `contracts/execution-contract.md` §1 — `register`, `appendEvent`, `complete`, `comment`, `proposeStatus`, `history`, `sync` (unit test: T1021). **No transport type and no data-access import may appear in this package**
- [ ] T1023 [P] Define the 29-event vocabulary as a discriminated union in `packages/execution-contract/src/events.ts` per `contracts/event-vocabulary.md`, with the four classes and the terminal-lifecycle set named once (unit test: T1024)
- [ ] T1024 [P] Write failing unit tests in `packages/execution-contract/tests/events.spec.ts` asserting every event has exactly one class, the nine lifecycle events are the only terminality-bearing ones, and the three withdrawn names (`validation-completed`, `status-proposed`, `reconciled`) are **absent** — reintroducing an ambiguous name is the regression this guards
- [ ] T1025 Write the failing migration test in `backend/tests/integration/executions/immutability.spec.ts` asserting, **separately for each immutable table**, that (a) `UPDATE` is rejected, (b) `DELETE` is rejected, each raising `'% is append-only'`; and that (c) a correction is representable **only** as a new superseding row and (d) a redaction **only** as a new event plus a state flag — never as an in-place edit (conformance: T1026). **Observe all four failing before `T1026` attaches the triggers**: the `reject_mutation()` FUNCTION exists, but **no EPIC-037 table is protected until a trigger is bound to it**
- [ ] T1080 [P] Write the failing test in `backend/tests/integration/executions/role-bypass.spec.ts` asserting the **application database role cannot bypass immutability** — it holds no `BYPASSRLS`, is not the table owner, is not `SUPERUSER`, and cannot `ALTER TABLE … DISABLE TRIGGER`. A protection the application can switch off is a convention, not a control (conformance: T1026)
...
- [ ] T1027 [P] Write the failing architecture test in `tests/architecture/connector-isolation.spec.ts` asserting no file under `packages/execution-contract/` or any connector path imports a store, a Prisma client or a `backend/src` module (conformance for `FR-EXR-019`; the mechanism that keeps `FR-AGT-004` honest today)
- [ ] T1028 Ship the fixture connector in `packages/execution-contract/src/fixture-connector.ts` — the conformance oracle every other connector is measured against (`R-037-10`) (unit test: T1029)
- [ ] T1029 [P] Write failing unit tests for the fixture connector in `packages/execution-contract/tests/fixture-connector.spec.ts`, driving all seven operations with no server present
...
- [ ] T1030 [P] [US1] Write failing contract tests for registration refusal conditions in `backend/tests/contract/executions/registration.spec.ts` — missing input version identity, unsupported contract version, unauthorised scope (covers T1032)
- [ ] T1031 [P] [US1] Write failing contract tests for phase-aware binding in `backend/tests/contract/executions/version-binding.spec.ts` per `V37-2` — **including `AC-EXR-17b`, that requiring `commitAfter` at registration is itself a defect** (covers T1033)
- [ ] T1032 [US1] Implement registration in `backend/src/modules/executions/execution-registration.service.ts` — root identity, correlation and idempotency keys, contract-version negotiation, `registered` event (contract test: T1030)
- [ ] T1033 [US1] Implement phase-aware target binding in `backend/src/modules/executions/execution-target.service.ts` — input required at registration, output required at successful completion, absent output legitimate on failure or cancellation (contract test: T1031)
- [ ] T1034 [P] [US1] Write failing unit tests for the frozen agent identity snapshot in `backend/tests/unit/executions/agent-identity.spec.ts` — renaming a descriptor afterwards must not alter history (`AC-EXR-14`) (covers T1035)
- [ ] T1035 [US1] Implement the agent identity snapshot, referencing EPIC-028's descriptor and freezing provider, model, adapter, version and capabilities (unit test: T1034)
- [ ] T1036 [US1] Implement completion in `backend/src/modules/executions/execution-registration.service.ts` — terminal event with outcome, artifacts, evidence references, validation results and a **mandatory** completion comment; an empty completion is a failed completion (contract test: T1037)
- [ ] T1037 [P] [US1] Write failing contract tests for completion in `backend/tests/contract/executions/completion.spec.ts` — each of the six terminal outcomes, and refusal of a success with no output binding
...
- [ ] T1039 [US1] Write the failing Constitution XI Tier 1 test in `backend/tests/integration/executions/round-trip.spec.ts` driving `V37-1` through the **real HTTP route against the composed module graph**, with the real composition root — a mocked collaborator does not satisfy this tier
- [ ] T1040 [P] [US1] Write failing integration tests for idempotency in `backend/tests/integration/executions/idempotency.spec.ts` per `V37-3` — replay returns the original, concurrent replays yield exactly one (covers T1041)
- [ ] T1041 [US1] Implement idempotency and correlation — `UNIQUE (workspaceId, idempotencyKey)` per `R-037-8`, replay returning the original record (integration test: T1040)
- [ ] T1042 [P] [US1] Write failing unit tests for argument sanitisation in `backend/tests/unit/executions/sanitisation.spec.ts` — a connection string in arguments never reaches storage or any response (`AC-EXR-10`) (covers T1043)
- [ ] T1043 [US1] Implement argument sanitisation and credential refusal in the registration path (unit test: T1042)
...
- [ ] T1044 [P] [US2] Write failing contract tests for terminality in `backend/tests/contract/executions/terminality.spec.ts` per `V37-4` — governance, content and reconciliation events **accepted** after a terminal lifecycle event; any lifecycle event **refused** (`AC-EXR-18`, `AC-EXR-19`) (covers T1045)
- [ ] T1045 [US2] Implement event append with class-aware terminality in `backend/src/modules/executions/execution-event.service.ts` (contract test: T1044)
- [ ] T1046 [P] [US2] Write failing integration tests for sequence allocation and optimistic concurrency in `backend/tests/integration/executions/projection.spec.ts` per `V37-8` — gapless server sequence, `expectedSequence` mismatch refused with the current value, a gap reported as a reconciliation fault (`AC-EXR-13`) (covers T1047, T1048)
- [ ] T1047 [US2] Implement server-side gapless sequence allocation with `expectedSequence` optimistic concurrency (`R-037-3`) (integration test: T1046)
- [ ] T1048 [US2] Implement the rebuildable projection in `backend/src/modules/executions/execution-projection.service.ts`, carrying `projectedThroughSequence` so staleness is visible (integration test: T1046). **Replay must reproduce stored state exactly** (`AC-EXR-12`)
- [ ] T1049 [P] [US2] Write failing unit tests for the comment model in `backend/tests/unit/executions/comments.spec.ts` — append-only, threaded, typed, access-scoped; a correction is a superseding comment, never an edit (covers T1050)
- [ ] T1050 [US2] Implement comments in `backend/src/modules/executions/execution-comment.service.ts` with the full field set of `data-model.md` §2 (unit test: T1049)
- [ ] T1051 [P] [US2] Write failing contract tests for redaction in `backend/tests/contract/executions/redaction.spec.ts` per `V37-9` — body concealed, tombstone recorded, integrity chain verifies before and after, **no hard-delete path reachable** (`AC-EXR-15`) (covers T1052)
- [ ] T1052 [US2] Implement redaction per `R-037-9` — immutable `comment-redacted` event, original retained under a named compliance role with each access audited; cryptographic erasure only where law compels (contract test: T1051)
- [ ] T1053 [P] [US2] Write failing integration tests for re-run linkage in `backend/tests/integration/executions/rerun.spec.ts` — a re-run creates a new linked execution and the parent stream is byte-identical afterwards (`AC-EXR-09`) (covers T1054)
- [ ] T1054 [US2] Implement re-run linkage via `parentExecutionId`; a terminal execution is never reopened (integration test: T1053)
...
- [ ] T1061 [US4] Implement the connector-side durable outbox in `packages/execution-contract/src/outbox.ts`, preserving `localSequence` and original `occurredAt` (unit test: T1060)
...
- [ ] T1068 [P] Write failing contract tests for MCP bindings in `packages/execution-contract/tests/mcp-binding.spec.ts` — `inputSchema` and `outputSchema` present, `structuredContent` returned, and **refusals as `isError: true` rather than JSON-RPC errors** (`R-037-7`) (covers T1069)
...
Every path touched is `packages/execution-contract/`, `backend/src/modules/executions/`,
