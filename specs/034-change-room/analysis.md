...
|---|---|---|
| C2 ✅ | Closed | `T406v` now cites `(conformance: T931 — on epic/030)`. The header note and plan Gate V both name `T931` explicitly, so the identifier is written in three places rather than described in one. Recorded as a **cross-Epic dependency**: `T406v` cannot be discharged until `epic/030` merges, because `T931` is the check and `T932` is what already reads every file in that directory |
...
| L1 ✅ | Closed | A fourth mutation-proof exit criterion added for `FR-CHR-062`, and criterion 5 now also requires the workflow file to pass `T931` |
