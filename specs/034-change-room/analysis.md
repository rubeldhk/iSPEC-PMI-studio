...
**Blocking**: `C2` (CRITICAL), `C1` and `A1` (HIGH) — **all resolved 2026-08-23**, see Remediation
status. `DOR-09` ignores rows marked ✅, so this record no longer holds the Epic out of `Ready`.
`DOR-06` still does, until `T406a` creates the worktree and the plan's Constitution Check row moves
FAIL → PASS.
...
| Total tasks | 98 → **99** after remediation |
| Requirement coverage | 100% |
| Exit-criteria coverage | 12/13 → **14/14** after remediation |
| Ambiguity count | 1 |
| Duplication count | 0 |
| Critical issues | 1 |
| High issues | 2 |
...
**Applied 2026-08-23** (the analysis ran 2026-08-22; the session crossed midnight, and the edits are
dated for the day they were made rather than the day they were recommended).

| ID | State | What changed |
|---|---|---|
| C2 ✅ | Closed | `T406v` now cites `(conformance: T993s — on epic/030)`. The header note and plan Gate V both name `T993s` explicitly, so the identifier is written in three places rather than described in one. Recorded as a **cross-Epic dependency**: `T406v` cannot be discharged until `epic/030` merges, because `T993s` is the check and `T932` is what already reads every file in that directory |
| C1 ✅ | Closed | **`T995l` added** — the `FR-CHR-054` mutation proof: silently retarget onto the newer baseline, require `T994h` to fail, record against `SC-CHR-009`. Placed with the other four mutation proofs rather than in letter order, per this file's own note that identifiers do not sort into execution order. Exit-criteria coverage is now **14/14** |
| A1 ✅ | Closed | `FR-CHR-040` now states that **every Change Request in this Room is a material change**, because `FR-CHR-011` makes this the only path an approved baseline changes and no threshold is representable in `ChangeRequest`. `FR-CHR-050` and `SC-CHR-003` cite that resolution; `T996r` states it at the point of test. `BR-0046`'s word is kept — what is added is which changes it selects |
| I1 ✅ | Closed | The plan's Project Structure gained the four files: `change-room-constraints`, `change-room-high-band`, `change-room-type-isolation`, `change-room-transcript`. **7 integration + 2 architecture**, matching tasks.md, with the drift recorded rather than the block silently extended |
| U1 ✅ | Closed | `T994z` rewritten to the standard failing-first form and the meaningless `T406u` citation dropped |
| L1 ✅ | Closed | A fourth mutation-proof exit criterion added for `FR-CHR-062`, and criterion 5 now also requires the workflow file to pass `T993s` |

### One thing the remediation itself surfaced

Closing `U1` made `T994z` a pure test-authoring task — and the inverse check immediately flagged it
as **a failing test with no implementation partner**. That was correct: nothing in this Epic
implements workflow-type isolation. It is a property of `change-room.json` and of `EPIC-030`'s
`T944b` resolution. So `T406v` now cites `(integration test: T994z)` and says so.

The detector caught a real gap introduced by a fix to a different finding, which is the argument for
running it after remediation rather than only before. Both halves of `U1` — the wrong citation and
the missing one — were invisible to a reader and visible to extraction.
...
`T995a`–`T995d` and `T995l` exist to close, one level up: a mutation proof asks whether the check
would notice. The pairing detector does not. Carried into `tasks.md`'s Notes so `EPIC-035` inherits
it rather than rediscovering it.

**Post-remediation verification** (2026-08-23): **99 tasks**, zero duplicate identifiers, zero
cross-branch collisions, `DOR-08` unpaired **0**, failing tests with no implementation partner **0**,
`FR`/`SC` uncited **0** of 51, quickstart scenarios named in full **14/14**, exit criteria covered
**14/14**, mutation proofs **5**. `FR` and `SC` counts are unchanged at **41** and **10** — the
`A1` fix resolves a term inside two existing requirements rather than adding one, so none of the
plan's seven extracted figures moved.
