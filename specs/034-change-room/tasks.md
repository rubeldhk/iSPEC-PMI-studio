...
- [ ] T406c [P] Add `backend/src/modules/change-room` to the `## Paths that must not break` list in `governance/repository-layout.md` (`G-05d`)
...
- [ ] T996v [P] [US3] Write failing component tests for the options region in `frontend/src/pages/ChangeRoom.options.test.tsx` — options render as recommendations, visually distinct from recorded fact and human decision (`FR-CHR-082`, `UX-0031`)
...
- [ ] T994r [P] [US6] Write failing component tests for the Room page in `frontend/src/pages/ChangeRoom.test.tsx` — six regions composed through the **imported** `RoomShell`, with loading, empty, populated and error states (`FR-CHR-080`, `UX-0051`)
...
- [ ] T994t [P] [US6] Write the failing region-vocabulary comparison test in `frontend/src/pages/ChangeRoom.vocabulary.test.tsx` — region names identical to `packages/room-contract` and to `EPIC-033`'s, verified by comparison rather than review (`FR-CHR-081`, `SC-CHR-008`)
- [ ] T994u [P] [US6] Write failing component tests for blockers and policy refusal in `frontend/src/pages/ChangeRoom.blockers.test.tsx` — what is blocking is visible without opening another screen, and a policy-refused action shows the refusing policy (`FR-CHR-083`, `FR-CHR-084`, `UX-0032`, `UX-0033`)
