...
- [ ] T701 [P] [US3] Write failing unit tests for proxy allowlist-config generation from an `EgressProfile` — exactly the profile's `allowedDestinations`, CONNECT-only, rejecting wildcard/empty via the same rules as `assertEgressProfile` — in `execution-providers/docker/tests/unit/proxy-config.spec.ts`
- [ ] T702 [US3] Implement the proxy config generator in `execution-providers/docker/src/proxy-config.ts`, consuming an `EgressProfile` and emitting the sidecar's allowlist config; NOT hand-written config, so profile and enforcement can never disagree (unit test: T701)
- [ ] T703 [US3] Create the proxy sidecar image definition in `execution-providers/docker/proxy/Dockerfile` with the base image pinned by digest in `engine-adapters/speckit/docker/pinned-versions.json` per RAID `R-01` conventions (conformance check: T704 reads the Dockerfile and the pin; a digest-less base fails it)
- [ ] T704 [P] [US3] Write the executable conformance check for the sidecar definition — Dockerfile exists, base pinned by digest, entrypoint consumes the T702-generated config — in `tests/governance/egress-proxy-image.spec.ts` (gates T703)
