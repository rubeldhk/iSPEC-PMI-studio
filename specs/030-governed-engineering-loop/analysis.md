...
# Analysis: EPIC-030 — C2C closure (`X7`, `X8`, `X11`)

**Session**: 2026-08-26 (second) · **Scope**: the three dependency findings and what closing them
surfaced. Read: EPIC-009, EPIC-021 and EPIC-014 `spec.md`, `tasks.md`, `closure.md`;
`specs/_shared/platform-spec.md`; and the implementations under
`backend/src/modules/{specifications,reviews,loop}`.

## Findings — Session 2026-08-26 (C2C)

| ID | Category | Severity | Location(s) | Summary | Recommendation |
|----|----------|----------|-------------|---------|----------------|
| X7 ✅ | Coverage Gap | HIGH | `backend/src/modules/reviews/` | **Closed.** EPIC-021 now has a Nest module, Prisma-backed gate configuration, an append-only authoritative decision table, and a public typed-disposition query. `UnconfiguredGateOutcomes` is out of the production graph | Verified end to end: a configured gate with a recorded human decision lets a proposal reach `applied` |
| X8 ✅ | Coverage Gap | HIGH | `backend/src/modules/specifications/` | **Closed.** State and evidence commit in **one transaction**; the expected status is part of the UPDATE's `WHERE`, so the concurrency check cannot be interleaved; `appliedTransitionId` is the committed id | Verified by seven proofs including real rollback, restart durability and application-role immutability |
| X11 ✅ | Underspecification | HIGH | `gate_final_outcomes` | **Closed.** Every decision binds tenant, specification, exact `from->to`, target version, gate-set version, frozen evaluator and decider identities, and correlation/causation. Any mismatch yields `stale` | Verified: adding a second gate makes an existing decision stale rather than authorising |
| X12 ✅ | Correctness | HIGH | `backend/tests/integration/loop/adjudication-composition.spec.ts` | **A C2A-closure test proved the wrong thing.** *"An ungranted proposal is refused by EPIC-024"* passed because EPIC-009 could not find the specification — `.rejects.toThrow()` accepted any rejection. EPIC-024 refuses with a deliberately **opaque** not-found, so the message cannot distinguish them | **Fixed.** The artifact is now governed by granting someone else, and the refusal is proven by the **access attempt EPIC-024 records** — the one thing only EPIC-024 does |
| X13 | Underspecification | MEDIUM | `backend/src/modules/access/` | EPIC-024's rule is **"unrestricted until granted"**: an artifact with no grants is editable by anyone in the workspace. Correct per `FR-ACC-*`, but it means an ungoverned specification is open, and its grant store is in-memory so grants do **not** survive a restart while transitions and gate decisions do | EPIC-037 must not assume a specification is protected merely because EPIC-024 is wired. Grant persistence is EPIC-014's wider deferral |
| X14 | Underspecification | LOW | `PrismaApplicationIntents`, `application_intents` | The durable-intent mechanism is **no longer on the production path** — the transaction is now the durability. The class, its table and its tests remain, and are unbound | Keep for a future non-transactional lifecycle owner, or retire under a named task. Not dead code, but no longer load-bearing |
| X2 | Constitution Alignment | MEDIUM | Constitution V | **Still open.** Failing-first not observed per pair | Unchanged |
| X10 ✅ | Ownership | HIGH | closure records; `specs/014-devops-release/tasks.md` | **Closed** by the owner's hybrid decision and by amending F-11.2 with the five composition tasks it lacked | — |

## Metrics

- Findings this session: **8** — 0 CRITICAL · 5 HIGH **closed** · 0 HIGH open · 2 MEDIUM · 1 LOW
- Tasks added: **13** across four Epics (`T1104`–`T1116`), each paired with its test

## Notes

`X12` is the one worth dwelling on. Two separate assertions in this programme have now passed for
...
| X19 ✅ | Underspecification | HIGH | `backend/src/modules/access/access-inheritance.service.ts` | **CLOSED in C2E**, by its owner EPIC-024 (`T1126`, `T1127`, `T1131`) and EPIC-009 (`T1128`). An artifact with no grants is refused; the workspace boundary is checked before grants; every specification is created with a human owner grant in the same transaction. | Done — see EPIC-024's C2E reopening record |
| X2 | Constitution Alignment | MEDIUM | Constitution V | **Still open.** Failing-first not observed per pair | Unchanged |
