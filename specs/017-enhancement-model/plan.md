...
**Tasks**: not yet generated · **Posture**: ⏸ **HELD** (decision D-10) — product capability
...
|---|---|---|---|
| [**EPIC-019** Steering Engine](../019-steering-engine/) | F-17.1 – F-17.3 | T225–T250 (26) | Delivers value with nothing else built. The only part touching the tenancy model, so it lands near EPIC-004's migration rather than paying for a second one |
...
| [**EPIC-021** Review Gates & Roles](../021-review-gates-roles/) | F-17.7, F-17.8, F-17.11 | T273–T295 (23) | Depends on the lifecycle (EPIC-009); carries the entire PP-017 cost exposure |
...
**87 tasks across four epics**, against the ~70 estimated below. The estimates were low mainly in
...
Task figures are **estimates**, not counts — `/speckit-tasks` has not run. Each assumes the mandatory
paired unit-test task (Constitution V).
...
- [ ] All tasks complete, every unit test passing (Constitution V)
- [ ] Quickstart **V17-1** to **V17-8** pass
- [ ] `pnpm test:arch` green — no engine-specific reference introduced by steering or roles
- [ ] RAID **R-02** re-scored with the twelve roles in scope
