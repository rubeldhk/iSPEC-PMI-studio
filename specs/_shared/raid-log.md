...
| **R-02** | **AI agent run cost is unbounded per job; a runaway job or a busy week produces a surprise bill** | H | H | **9** | Hard wall-clock, CPU, and memory caps per container (R-006); surface job duration; add per-workspace budget alerting before production | Tech lead |
