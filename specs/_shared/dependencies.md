...
| **D-31** | **react-markdown** | **10.x** (pinned exactly at `10.1.0`, `EPIC-045` `T1628`) | Renders synced markdown to React elements without an HTML string (`EPIC-045` `R-045-5`, `FR-ART-020`–`FR-ART-025`). **Sanitising strategy**: no `rehype-raw`, so raw HTML is escaped as text; `defaultUrlTransform` blocks `javascript:`/`data:`/`vbscript:`; a `urlTransform` drops every `src` so nothing is fetched; `img` renders alternative text. Context7 `/remarkjs/react-markdown` | MIT | ☑ | Low — maintained by the remark ecosystem; the one new dependency PMI-DOC-007 §10 anticipated |
| **D-32** | **remark-gfm** | **4.x** (pinned exactly at `4.0.1`, `EPIC-045` `T1628`) | GitHub-flavoured markdown for the same renderer: tables, task lists, strikethrough — what every `tasks.md` and spec table uses (`EPIC-045`) | MIT | ☑ | Low — same family as D-31; adds no HTML path |
