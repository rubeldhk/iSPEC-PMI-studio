...
describe('T1359 · the scope registry', () => {
  it('holds exactly the fourteen scopes of record', () => {
    ...
    expect(registeredConnectorScopes()).toEqual(['artifacts.sync', 'connector.whoami', 'constitution.read', 'decomposition.read', 'execution.append', 'execution.comment', 'execution.complete', 'execution.propose', 'execution.read', 'execution.register', 'execution.sync', 'health.write', 'project.read', 'requirements.read', 'tasks.sync']);
  });
  ...
  it('refuses a route that declares no scope, and one whose scope is not registered — 403, after the credential is verified', async () => {
    ...
    // EPIC-046 T1693. The example has moved twice: `artifacts.sync` until
    // EPIC-045 T1626 made it permanent, then `tasks.sync` until EPIC-046 T1692
    // did the same. **No reserved-but-unregistered scope remains** — the two
    // tools still reserved (`pmi.execution.sync`) ride a scope that is already
    // registered — so borrowing a future Epic's name is no longer possible and
    // would only move the problem again. This is a synthetic name that will
    // never be registered, and the assertion below is what keeps it that way.
    expect(isRegisteredConnectorScope('never.registered')).toBe(false);
    await expect(h.guard.authenticate(request(`Bearer ${h.minted.value}`), 'never.registered')).rejects.toBeInstanceOf(ForbiddenError);
  ...
  it('can be extended by a later Epic without touching the guard (U1)', async () => {
    const h = await harness();
    // The same demonstration with a synthetic scope no Epic will ever take:
    // registering it is the whole change, and the guard is not touched.
    // (`artifacts.sync` was the example until EPIC-045 T1626 made it permanent,
    // then `tasks.sync` until EPIC-046 T1692 did — hence a name that is not a
    // future Epic's to claim.)
    registerConnectorScope('never.registered');
    try {
      await expect(h.guard.authenticate(request(`Bearer ${h.minted.value}`), 'never.registered')).resolves.toMatchObject({ projectId: 'p_a' });
    } finally {
      registerConnectorScope('never.registered', { remove: true });
    }
    expect(registeredConnectorScopes()).toEqual(['artifacts.sync', 'connector.whoami', 'constitution.read', 'decomposition.read', 'execution.append', 'execution.comment', 'execution.complete', 'execution.propose', 'execution.read', 'execution.register', 'execution.sync', 'health.write', 'project.read', 'requirements.read', 'tasks.sync']);
  });
