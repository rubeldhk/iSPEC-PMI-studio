 ...
 */
const service = async (policy: ChangeDecisionPolicyPort) =>
  new DecisionService(await storeWithImpactView(), policy);
const unboundService = async () => new DecisionService(await storeWithImpactView(), undefined);

describe('T996t · what a decision retains', () => {
  it('the option chosen', async () => {
    const decision = await (await service(permits)).record(input());
...
describe('T996t · what it refuses', () => {
  it('a chosen option that was never offered', async () => {
    ...
    await expect((await service(permits)).record(input({ chosenOptionId: 'z' }))).rejects.toThrow(
  ...
  it('an empty rationale', async () => {
    ...
    await expect((await service(permits)).record(input({ rationale: '   ' }))).rejects.toThrow(/rationale/i);
  });

  it('a decision by a non-human', async () => {
    // `SC-CHR-003` — zero auto-approvals under any tenant policy. `RULE-03`.
    await expect((await service(permits)).record(input({ decidedByKind: 'ai' }))).rejects.toThrow(/human/i);
  });
...
describe('T996t · an unbound policy provider refuses', () => {
  it('rather than recording an unauthorised decision', async () => {
    ...
    // recorded yet.
    await expect((await unboundService()).record(input())).rejects.toThrow(/EPIC-031/);
  });

  it('and the refusal names the seam rather than blaming the caller', async () => {
    await expect((await unboundService()).record(input())).rejects.toThrow(/no policy provider is bound/i);
  });

  it('a policy that declines is refused too, with its reason', async () => {
    await expect((await service(refuses)).record(input())).rejects.toThrow(/no authority/i);
  });
...
describe('T996t · the authority basis is recorded, not assumed', () => {
  ...
  it('and never a default when the provider gives none', async () => {
    ...
    await expect((await service(vague)).record(input())).rejects.toThrow(/authority basis/i);
  });
});
