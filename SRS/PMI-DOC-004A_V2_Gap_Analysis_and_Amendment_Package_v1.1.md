...
**Document ID**: PMI-DOC-004A · **Version**: 1.1 · **Status**: ACCEPTED — actioned by PMI-DOC-004 v2.0
**Owner**: Project Owner (Product) · **Date**: 2026-08-21
**Baseline reviewed**: PMI-DOC-004 v1.0, PMI-DOC-005 v1.0, prior PMI Studio MPS/module
specifications, EPIC-001 system design, the accepted Augment/Cosmos Learnings Amendment, the Kiro
enhancement direction, and current external product research.

---

## 0. Disposition — read first

This document was accepted on 2026-08-21 and is **actioned** by
[PMI-DOC-004 v2.0](./PMI-DOC-004_Business_Requirement_Specification_v2.0.md). It is retained as the
reasoning record: v2.0 states what the product requires, this states *why the change was necessary*.

The six decisions in §14 were approved as written. Two defects were found during implementation that
this analysis did not catch, and both were corrected in v2.0 rather than carried:

| Finding | Detail | Where corrected |
|---|---|---|
| **F-01 — identifier collision** | The circulated v2.0 draft reused ten approved v1.0 `BR-` identifiers with different meanings (`BR-0040`, `0041`, `0050`, `0060`, `0061`, `0062`, `0063`, `0080`, `0090`, `0100`). PMI-DOC-000 §3 makes identifiers corpus-wide, and 50 live citations across `specs/`, `governance/` and `adr/` depended on the v1.0 meanings. The draft would have silently re-meant published identifiers and failed its own acceptance criterion 2 | PMI-DOC-004 v2.0 §0.1 (ruling), §14 (crosswalk), `RULE-16` |
| **F-02 — coverage regression** | The circulated draft dropped business coverage for `EPIC-018`, `EPIC-025` and `EPIC-026`, orphaning three declared Epics | PMI-DOC-004 v2.0 §0.2, §6.17, §6.21 |

**F-01 is the more instructive of the two.** This analysis correctly identified `G-36` — a
stated-versus-actual count defect in v1.0 — and then produced a draft carrying a larger instance of
the same class of error. That is why v2.0 §12 mechanizes three identifier assertions instead of
relying on review.

Two further observations were recorded but **not** actioned here, because both belong to documents
this package does not own:

- `specs/027-ai-native-amendment/register/capability-areas.md` contains its `## The twenty areas`
  section twice, duplicated verbatim. That file is digest-checked by `G-27-11`, so it must be fixed
  through `pnpm register:build`, not by hand.
- The same register assigns "Persistent project state" to *"EPIC-029 (proposed)"*, but `EPIC-029` was
  subsequently declared as Design System. That capability area now has a stale home.

Both are recorded in [`specs/brs-v2-reconciliation.md`](../specs/brs-v2-reconciliation.md) §5.
