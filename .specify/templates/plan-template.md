...
| XII | Every governed Spec Kit command this plan causes to run is **registered in PMI Studio** before it executes, whatever surface originates it — sandbox, IDE, CLI, CI or connector. Execution history is planned as **append-only immutable events**, never mutable updates; target binding is **phase-aware** (input identity at registration, output identity at completion); status transitions are **proposed by the agent and adjudicated by the platform**, never applied by a connector. An Epic that causes no governed execution records that, rather than omitting the row | [PASS/FAIL] |
